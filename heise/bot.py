...
async def get_entry_count(table="heise"):
    """Zählt Einträge in der angegebenen Tabelle (heise oder chip)"""
    try:
...
async def get_author_count(table="heise"):
    """Zählt einzigartige Autoren in der angegebenen Tabelle"""
    try:
        conn = await asyncpg.connect(**DB_CONFIG)
...
async def get_today_counts(table="heise"):
    """Zählt heutige Artikel und Autoren in der angegebenen Tabelle"""
    try:
        conn = await asyncpg.connect(**DB_CONFIG)
...
async def search_articles_by_author(author_name, keywords=None, category=None, limit=10, table="heise"):
    ...
    try:
        ...
        if category:
            param_count += 1
            # Für chip-Tabelle gibt es unterschiedliche Kategoriefelder
            if table == "chip":
                conditions.append(f"(page_level1 ILIKE ${param_count} OR page_level2 ILIKE ${param_count} OR page_level3 ILIKE ${param_count})")
            else:
        ...
        # Spaltennamen anpassen je nach Tabelle
        if table == "chip":
            query = f"""
                SELECT title, url, date, author, page_level1 as category 
                FROM {table} 
                WHERE {where_clause}
                ORDER BY date DESC 
                LIMIT ${param_count}
        ...
        print(f"❌ Fehler bei der Autoren-Suche ({table}): {e}")
        return []
